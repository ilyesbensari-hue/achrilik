...
export default function Navbar() {
    ...
    const { tr, lang, isRTL } = useTranslation();
    ...
    useEffect(() => {
        const saved = localStorage.getItem('achrilik_dark');
        if (saved === '1') {
            setIsDark(true);
    ...
    const toggleDark = () => {
        ...
        localStorage.setItem('achrilik_dark', next ? '1' : '0');
    };
    ...
    useEffect(() => {
        ...
        // Listen to lang changes from LanguageContext
        const handleLangChange = (e: Event) => {
            const newLang = (e as CustomEvent<'fr' | 'ar'>).detail;
            if (newLang === 'fr' || newLang === 'ar') setCurrentLang(newLang);
        };
        ...
        return () => {
            ...
            window.removeEventListener('achrilik_lang_change', handleLangChange);
        };
    }, []);
    ...
    return (
        <nav className={`sticky top-0 z-50 transition-all duration-300 ${scrolled
            ...
            : 'bg-white shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border-b border-gray-100'
            }`} dir={isRTL ? 'rtl' : 'ltr'}>
            <div className="max-w-7xl mx-auto flex items-center justify-between h-16 px-4 gap-3">
                ...
                <div className="flex md:hidden items-center justify-between w-full gap-3">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        <button
                            onClick={toggleLang}
                            className="px-2 py-1 text-xs font-bold text-[#006233] border border-[#006233] rounded-full hover:bg-[#006233] hover:text-white transition-colors"
                        ...
                        >
                            {isDark ? (
                                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.343l-.707-.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" strokeLinecap="round" strokeLinejoin="round" /></svg>
                            ) : (
                ...
                <div className="hidden md:flex items-center justify-between w-full gap-4">
                    ...
                    <div className="relative group flex-shrink-0">
                        ...
                        {/* Dropdown Menu */}
                        <div className={`absolute ${isRTL ? 'right-0' : 'left-0'} top-full mt-2 w-64 bg-white rounded-lg shadow-xl border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50`}>
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                        {!user ? (
                            ...
                            <div className="relative group">
                                ...
                                {/* User Dropdown */}
                                <div className={`absolute ${isRTL ? 'left-0' : 'right-0'} top-full mt-2 w-56 bg-white rounded-lg shadow-xl border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 py-2 z-50`}>
                                    <Link
                                        href="/profile"
                                    ...
                                    >
                                        ...
                                        <span className="text-sm font-medium text-gray-700">{tr('nav_profile')}</span>
                                    </Link>
                                    ...
                                    {(user.role === 'SELLER' || (Array.isArray(user.roles) && user.roles.includes('SELLER'))) && (
                                        ...
                                        >
                                            ...
                                            <span className="text-sm font-medium text-gray-700">{tr('nav_myshop')}</span>
                                        </Link>
                                    )}
                                    ...
                                    {(user.role === 'ADMIN' || (Array.isArray(user.roles) && user.roles.includes('ADMIN'))) && (
                                        ...
                                        >
                                            <span className="text-lg">👑</span>
                                            <span className="text-sm font-bold text-purple-700">{tr('nav_admin')}</span>
                                        </Link>
                                    )}
                                    ...
                                    <button
                                        ...
                                        className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-red-50 transition-colors ${isRTL ? 'text-right' : 'text-left'}`}
                                    >
                                        <svg className="w-5 h-5 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                                        </svg>
                                        <span className="text-sm font-medium text-red-600">{tr('nav_logout')}</span>
                                    </button>
                                </div>
                            </div>
                        )}
                        ...
                        <button
                            ...
                            aria-label="Changer de langue / تغيير اللغة"
                            title={currentLang === 'fr' ? 'النسخة العربية' : 'Version Française'}
                        >
            ...
            {mobileMenuOpen && (
                <>
                    ...
                    <div className={`lg:hidden fixed top-0 bottom-0 ${isRTL ? 'right-0' : 'left-0'} z-[61] w-[85%] max-w-xs bg-white shadow-xl transform transition-transform duration-300 ease-in-out overflow-y-auto overscroll-contain h-[100dvh]`}
                        dir={isRTL ? 'rtl' : 'ltr'}>
                        {/* Header */}
                        <div className="flex items-center justify-between p-4 border-b border-gray-100 sticky top-0 bg-white z-10">
                            <h3 className="text-lg font-bold text-[#006233]">{tr('nav_menu')}</h3>
                            <button
                                onClick={() => setMobileMenuOpen(false)}
                        ...
                        <div className="p-4">
                            <div className="space-y-6">
                                {/* Mobile Auth/Actions */}
                                <div>
                                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 px-1">{tr('nav_account')}</h3>
                                    {user ? (
                                        <div className="space-y-3">
                                            ...
                                            >
                                                👋 {user.name} <span className="text-xs text-gray-400 ms-1">{tr('nav_access_profile')}</span>
                                            </Link>
                                            {user.role === 'ADMIN' && (
                                                <Link
                                                    href="/admin"
                                            ...
                                            <button
                                                onClick={handleLogout}
                                                className={`w-full ${isRTL ? 'text-right' : 'text-left'} p-3 rounded-xl bg-gray-50 text-red-600 font-medium hover:bg-red-50 transition-colors`}
                                            >
                                                {tr('nav_logout')}
                                            </button>
                                        </div>
                                ...
                                {user && user.role === 'SELLER' && (
                                    <div>
                                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 px-1">{tr('nav_sell')}</h3>
                                        <Link
                                            href="/sell"
                                            onClick={() => setMobileMenuOpen(false)}
                                            className="flex items-center gap-3 p-3 rounded-xl bg-green-50 text-green-700 font-medium border border-green-100"
                                        >
                                            <span>📦</span> {tr('nav_myproducts')}
                                        </Link>
                                    </div>
                                )}
                                ...
                                {/* Categories */}
                                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3 px-1">{tr('nav_explore')}</h3>
