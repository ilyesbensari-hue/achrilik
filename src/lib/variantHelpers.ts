...
export function getSizeConfig(categorySlug?: string, categoryName?: string): SizeConfig {
    ...
    // ==========================================
    // ÉLECTRONIQUE & TECH — slugs réels DB
    // ==========================================
    const TECH_SLUGS = [
        'electronique', 'telephones-accessoires', 'batteries-energie',
        'audio', 'supports-gadgets',
        'coques', 'coques-silicone', 'coques-rigide', 'coques-anti-choc',
        'protege-ecrans', 'power-banks', 'cables-charge',
        'chargeurs-secteur', 'chargeurs-voiture', 'chargeurs-voiture',
        'ecouteurs-filaires', 'ecouteurs-sans-fil', 'casques-audio',
        'supports-telephone', 'anneaux-maintien',
    ];

    const isTech = TECH_SLUGS.includes(slug) ||
    ...
    // ==========================================
    // CHAUSSURES — slugs réels DB
    // ==========================================
    const SHOE_SLUGS_HOMME = ['baskets-homme', 'souliers-homme', 'sandales-homme', 'chaussures-hommes'];
    const SHOE_SLUGS_FEMME = ['baskets-femme', 'escarpins-femme', 'sandales-femme', 'bottes-femme', 'chaussures-femmes'];
    const SHOE_SLUGS_ENFANT = ['chaussures-enfants'];

    const isShoeHomme = SHOE_SLUGS_HOMME.includes(slug) ||
        (combined.includes('chaussure') && combined.includes('homme')) ||
        (combined.includes('basket') && combined.includes('homme')) ||
        (combined.includes('soulier'));

    const isShoeFemme = SHOE_SLUGS_FEMME.includes(slug) ||
        (combined.includes('chaussure') && combined.includes('femme')) ||
        (combined.includes('basket') && combined.includes('femme')) ||
        combined.includes('escarpin') || combined.includes('botine') || combined.includes('botte');

    const isShoeEnfant = SHOE_SLUGS_ENFANT.includes(slug) ||
        (combined.includes('chaussure') && (combined.includes('enfant') || combined.includes('bebe')));
    ...
    // Chaussures générique (catégorie parent)
    if (slug === 'chaussures' || combined.includes('chaussure')) {
        return {
            options: [
